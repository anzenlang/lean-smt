...
structure Reconstruct.State (ω : Prop) where
  termCache : Std.HashMap (cvc5.Term ω) Expr := {}
  proofCache : Std.HashMap (cvc5.Proof ω) Expr := {}
  count : Nat := 0
  currAssums : Array Expr := #[]
  skippedGoals : Array MVarId := #[]

abbrev ReconstructM (ω : Prop) := ReaderT Reconstruct.Context (StateT (Reconstruct.State ω) MetaM)

abbrev SortReconstructor := {ω : Prop} → cvc5.Srt ω → ReconstructM ω (Option Expr)

abbrev TermReconstructor := {ω : Prop} → cvc5.Term ω → ReconstructM ω (Option Expr)

abbrev ProofReconstructor := {ω : Prop} → cvc5.Proof ω → ReconstructM ω (Option Expr)
...
private unsafe def getReconstructorsUnsafe (n : Name) (rcons : Type)
: MetaM (List (rcons × Name)) := do
  let env ← getEnv
...
def reconstructSort (s : cvc5.Srt ω) : ReconstructM ω Expr := do
  let rs ← getReconstructors ``SortReconstructor SortReconstructor
  go rs s
where
  go (rs : List (SortReconstructor × Name)) (s : cvc5.Srt ω) : ReconstructM ω Expr := do
    for (r, n) in rs do
...
def reconstructSortLevelAndSort (s : cvc5.Srt ω) : ReconstructM ω (Level × Expr) := do
  let t ← reconstructSort s
  let .sort u ← Meta.inferType t | throwError "expected a sort, but got\n{t}"
  return ⟨u, t⟩

def withNewTermCache (k : ReconstructM ω α) : ReconstructM ω α := do
...
def traceReconstructTerm
  (t : cvc5.Term ω) (r : Except Exception Expr)
: ReconstructM ω MessageData :=
  return m!"{t} ↦ " ++ match r with
    | .ok e    => m!"{e}"
    | .error _ => m!"{bombEmoji}"

def reconstructTerm : cvc5.Term ω → ReconstructM ω Expr := withTermCache fun t => do
...
where
  withTermCache
    (r : cvc5.Term ω → ReconstructM ω Expr) (t : cvc5.Term ω)
  ...
  reconstruct r t := do
    ...
    return e
  go (rs : List (TermReconstructor × Name)) (t : cvc5.Term ω) : ReconstructM ω Expr := do
    for (r, n) in rs do
...
open Qq in
def reconstructTerms {u} {α : Q(Type $u)} (ts : Array (cvc5.Term ω)) : ReconstructM ω Q(List $α) :=
  let f := fun t ys => do
    let a : Q($α) ← reconstructTerm t
    return q($a :: $ys)
  ts.foldrM f q([])

def withNewProofCache (k : ReconstructM ω α) : ReconstructM ω α := do
...
def withProofCache
  (r : cvc5.Proof ω → ReconstructM ω Expr) (pf : cvc5.Proof ω)
: ReconstructM ω Expr := do
...
def incCount : ReconstructM ω Nat :=
  modifyGet fun state => (state.count, { state with count := state.count + 1 })

def withAssums (as : Array Expr) (k : ReconstructM ω α) : ReconstructM ω α := do
...
def findAssumWithType? (t : Expr) : ReconstructM ω (Option Expr) := do
...
def skipStep (mv : MVarId) : ReconstructM ω Unit := mv.withContext do
  let state ← get
...
def addThm (type : Expr) (val : Expr) : ReconstructM ω Expr := do
...
def addTac (type : Expr) (tac : MVarId → MetaM Unit) : ReconstructM ω Expr := do
  let name := Name.num `s (← incCount)
  let mv ← Meta.mkFreshExprMVar type .natural name
  tac mv.mvarId!
...
def addTrust (type : Expr) (pf : cvc5.Proof ω) : ReconstructM ω Expr := do
  let name := Name.num `s (← incCount)
  let mv ← Meta.mkFreshExprMVar type .natural name
  skipStep mv.mvarId!
...
def traceReconstructStep (r : Except Exception Expr) : ReconstructM ω MessageData :=
  return match r with
  | .ok _ => m!"{checkEmoji}"
  | _     => m!"{bombEmoji}"

partial def reconstructProof : cvc5.Proof ω → ReconstructM ω Expr := withProofCache fun pf => do
  let rs ← getReconstructors ``ProofReconstructor ProofReconstructor
  go rs pf
where
  go (rs : List (ProofReconstructor × Name)) (pf : cvc5.Proof ω) : ReconstructM ω Expr :=
...
open Qq in
partial def reconstructProof (pf : cvc5.Proof ω) (ctx : Reconstruct.Context) :
...
open cvc5 in
def traceSolve (r : Except Exception (Except SolverError α)) : MetaM MessageData :=
  return match r with
...
open cvc5 in
def solve
  (query : String) (timeout : Option Nat)
  (proofDo : {ω : Prop} → cvc5.Proof ω → EnvT ω MetaM α)
: MetaM (Except Error α) :=
  profileitM Exception "simp" {} do
  withTraceNode `smt.solve traceSolve do
    -- Solver.run (← TermManager.new) do
    cvc5.run (m := MetaM) fun {ω} => do
      let solver : Solver ω ← Solver.new (ω := ω)
      ...
      if r.isUnsat then
        ...
        if h : 0 < ps.size then
          let proof := ps[0]
          let proofString ← solver.proofToString proof
          trace[smt.solve] "proof:\n{proofString}"
          let res ← proofDo proof
          return res
        else cvc5.throw s!"Unexpected empty array of proofs"
      else cvc5.throw s!"Expected unsat, got {r}"
...
@[tactic reconstruct] def evalReconstruct : Tactic := fun stx =>
  withMainContext do
    ...
    let res ← solve query none fun pf => do
